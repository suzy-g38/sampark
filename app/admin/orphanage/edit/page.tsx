...
const OrphanageUpdate = () => {
  ...
  useEffect(() => {
    ...
    if (!initialLoading && orphanageData && !formInitialized) {
      form.setValues({
        name: orphanageData.name || '',
        location: orphanageData.location || '',
        contactInformation: orphanageData.contactInformation || '',
        vision: orphanageData.vision || '',
        description: orphanageData.description || '',
        capacity: orphanageData.capacity || 0,
        servicesProvided: orphanageData.servicesProvided || [],
        startAge: orphanageData.startAge || 0,
        endAge: orphanageData.endAge || 0,
        logo: orphanageData.logo || '',
        operatingHours: orphanageData.operatingHours || '',
        license: orphanageData.license || '',
        staffInformation: orphanageData.staffInformation || [],
        donationInformation: orphanageData.donations || {
          isDonations: false,
          donationContact: '',
        },
        testimonials: orphanageData.testimonials || [],
      });
      setFormInitialized(true);
    }
  }, [orphanageData, initialLoading, formInitialized]);

  const getData = async () => {
    setInitialLoading(true);

    try {
      const response = await fetch(`/api/orphanage/id`, {
        method: 'GET',
      }).then((res) => res.json());
      console.log(response);
      if (response.success) {
        setOrphanageData(response.orphanage);
        setStaffDetails(response.orphanage.staffInformation);
        setDonationDetails(response.orphanage.donations);
        setTestimonialDetails(response.orphanage.testimonials);
  ...
  const registerHandler = async () => {
    ...
    try {
      const response = await fetch('/api/orphanage/add', {
        method: 'POST',
...
export default OrphanageUpdate;
