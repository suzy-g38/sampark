...
const NGOUpdate = () => {
  ...
  useEffect(() => {
    // Set the form values when ngoData is updated
    if (!initialLoading && ngoData && !formInitialized) {
      form.setValues({
        name: ngoData.name || '',
        ...
        volunteering: volunteeringDetails || {},
        logo: ngoData.logo || '',
        testimonials: testimonialDetails || [],
        socialMediaLinks: socialMedia || {},
        license: ngoData.license || '',
        funding: ngoData.funding || '',
      });
      setFormInitialized(true);
    }
  }, [ngoData, initialLoading, formInitialized]);
  ...
  const getData = async () => {
    ...
    try {
      ...
      if (response.success) {
        setNgoData(response.ngo);
        setContactInfomation(response.ngo.contactInformation);
        setProjectInformation(response.ngo.projects);
        setTeamMembers(response.ngo.teamMembers);
        setDonationDetails(response.ngo.donations);
        setVolunteeringDetails(response.ngo.volunteering);
        setTestimonialDetails(response.ngo.testimonials);
        setSocialMedia(response.ngo.socialMediaLinks);
  ...
  const registerHandler = async () => {
    ...
    try {
      const response = await fetch('/api/ngo/edit', {
        method: 'POST',
        body: JSON.stringify({
          data: form.values,
        }),
          ...
          <Stepper active={active} breakpoint="sm">
            ...
            >
              ...
              <TextInput
                ...
                type="number"
                onChange={(e) => {
                  setContactInfomation((prevValue) => ({
                    ...prevValue,
